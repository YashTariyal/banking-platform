...
/**
 * Fail fast if kyc-service is started with security enabled but
...
public class SecurityStartupValidator {
    ...
    public SecurityStartupValidator(Environment environment,
                                    @Value("${kyc.security.enabled:false}") boolean securityEnabled) {
    ...
    public void validateSecurityConfiguration() {
        ...
        if ((jwkSetUri == null || jwkSetUri.isBlank())
            ...
            throw new IllegalStateException("""
                    kyc.security.enabled=true but no JWT configuration found.
